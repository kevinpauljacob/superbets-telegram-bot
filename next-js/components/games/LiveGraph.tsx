...
export default function LiveGraph({
  ...
  const { liveStats: data, liveCurrentStat, selectedCoin } = useGlobalContext();
  ...
  useEffect(() => {
    ...
    const positiveD = fdata.map((item) => ({
      ...item,
      totalPNL: item.totalPNL > 0 ? item.totalPNL : 0,
    }));
    const negativeD = fdata.map((item) => ({
      ...item,
      totalPNL: item.totalPNL < 0 ? item.totalPNL : 0,
    }));
