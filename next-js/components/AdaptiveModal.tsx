  ...
  <DialogPortal>
    ...
    >
      ...
      <DialogPrimitive.Close className="absolute right-4 top-6 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
        <IoCloseOutline className="w-8 h-8 cursor-pointer text-[#fcfcfc] hover:bg-[#26282c] transition-all  rounded-full" />
...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ...
      ref={ref}
      className={cn(
        "fixed inset-x-0 bottom-0 z-[150] mb-16 flex flex-col border-[#121418] outline-none",
        className,
      )}
      {...props}
    >
      <div className="mx-auto h-1 w-10 mt-4 mb-2 rounded-full bg-gray-400" />
      {children}
    </DrawerPrimitive.Content>
  </DrawerPortal>
));
